...
### Local sessions (top)
Current project's sessions with full interactive control. Sessions modified within `recent_threshold` minutes (default 10) sort at the top, running (managed) sessions first and then alphabetically by name; older sessions sort by most recent first. Hidden sessions always sort in the older tier. Rescanned every 1 second.
